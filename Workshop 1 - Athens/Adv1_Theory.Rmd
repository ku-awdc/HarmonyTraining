...
rmarkdown::render('Adv1_Theory.Rmd', 'beamer_presentation', params=list(solutions=FALSE))
# And for html:
rmarkdown::render('Adv1_Theory.Rmd', 'html_document', params=list(solutions=TRUE))
```
...
`r if(!params$solutions) {"\\begin{comment}"}`
...
`r if(!params$solutions) {"\\end{comment}"}`
...
run.jags(mt, n.chains=2)
```

- But this makes the model harder to read, so I would generally stick to using a separate text file!
...
- For a complete list of the distributions available see:  https://sourceforge.net/projects/mcmc-jags/files/Manuals/4.x/
...
imperfect_definition <- "model{
  Positives ~ dbinom(obsprevalence, TotalTests)
  obsprevalence <- (prevalence * se) + ((1-prevalence) * (1-sp))
  ...
  # Hooks for automatic integration with R:
  #data# Positives, TotalTests
  #monitor# prevalence, obsprevalence, se, sp
...
findbeta(themean=0.9, percentile = 0.975, percentile.value = 0.8)
```

```{r}
curve(dbeta(x, 41.82, 4.65), from=0, to=1)
qbeta(c(0.025, 0.975), 41.82, 4.65)
```
